...
# # 初始化函数
def init_msdk(currentConfig, json_config):
    global curConfig
    ...
    #     print(f"初始化失败: {e}")
    print(json_config)
    callback_finish(MSDKStatus.MSDK_SUCCESS_INIT.value, "初始化成功", "asdqwdqwdqw123456")
...
# 定义MSDK_SpeakByAudio函数
# msdk_dll.MSDK_SpeakByAudio.argtypes = [ctypes.c_char_p, ctypes.POINTER(MSDKJsonParamsSpeakByAudio), MSDKCallback_SpeakByAudio]
# msdk_dll.MSDK_SpeakByAudio.restype = None
...
def speak_by_audio(client_id, config):
    ...
    # msdk_dll.MSDK_SpeakByAudio(client_id, ctypes.byref(json_config), callback)
    # typedef struct MSDKSpeakByAudio {
    #     int FrameNum; // 一帧数据量，建议为8320
    #     int FrameID;// 帧ID
    #     const char* Subtitle;// 字幕
